...
\newcommand{\degoffset}{\ensuremath{\tabsize-1-(\witsize-2)}\xpsace}
\begin{document}
    \maketitle
\begin{abstract}
...
\begin{enumerate}
 \item \prv computes the polynomial $m(X)\in \polysofdeg{\tabsize}$ defined by setting $m_i$, for each $i\in [\tabsize]$, to the number of times $\tab_i$ appears in \restricttoset{f}{\subspace}.
...
point is that when using an SRS built with higher degrees in \G1, $A$ must also be degree checked via an additional pairing. In such a case, we must also change the power of $X$ in Step \ref{step:checkadeg} from \degoffset to $d-(\witsize-2)$ where $d$ is the maximal SRS degree in \G1.}
...
We thank Kobi Gurkan and Aurel Nicolas for corrections.
