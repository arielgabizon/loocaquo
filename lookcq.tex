...
The result crucially relies on the following lemma using a result of Feist and Khovratovich\cite{fastkzgproofsorig}.
...
\begin{proof} 
    ...
    We can write the quotient $Q_i(X)$ as the KZG opening for $T(\vgen^i) = T_i$ scaled by
    the derivative of $Z_\bigspace$ at $\vgen^i$.
...
elements of \subspace.
    Thus, the algorithm of Feist and Khovratovich \cite{fastkzgproofsorig, fastkzgproofs} 
    can be used to compute commitments to all the proofs $\enc1{K_i(X)}$ for a
