...
We give a natural definition of a lookup protocol for algerbic adversaries.
\begin{dfn}\label{dfn:lookupprot}
A lookup protocol as a tuple $(\gen,\isintable)$ such that
\begin{itemize}
 \item $\gen$: is a randomized algorithm receiving as input parameters $\tabsize,\witsize,\tab\in \F^\tabsize,d$ for $d\geq \tabsize$.
 ...
 \item $\isintable$ an interactive public coin protocol between \prv and \ver where \prv has input $f\in \polysofdeg{\witsize}$, \ver has input $\cm$.
 Both \prv and \ver have $\witsize,\tab$ and $\srs=\gen(\tabsize,\witsize,\tab,d)$.
 such that
 \begin{itemize}
  \item Completeness:If $\cm=\enc1{f}$ and $\restricttoset{f}{\subspace}\subset \tab$ then \ver outputs \acc with probability one.
  \item Knowledge soundness in the algebraic group model:
    The probability of any algebraic \adv to win the following game is \negl

\begin{enumerate}
\item \adv chooses  $\tabsize,\witsize,\tab\in \F^\tabsize$.
...
\end{dfn}
Main protocol:
$\gen(\tab,\tabsize,\witsize)$:
\begin{enumerate}
\item Choose random $x\in \F$ compute and outputs \sett{\enc1{x^i},\enc2{x^i}}{i\in \set{0,\ldots,\tabsize}}
...
% \enc2{Z_\bigspace(x)}, \enc2{T(x)}
% Input $(\cm,f)$.
% 	\paragraph{\textbf{Round 1:} Committing to the multiplicities vector, and $f$ in \G2} \ \\
% \begin{enumerate}
%  \item \prv computes poly $m\in \polysofdeg{\tabsize}$ such that $m_i=$ number of times $\tab_i$ appears in \restricttoset{f}{\subspace}
%  \item \prv sends $\m\defeq \enc1{m(x)}$.
