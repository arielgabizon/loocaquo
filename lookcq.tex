...
\item The construction can be altered to a version we call \cqstar so that we lose the mentioned aggregability, increase preprocessing time to $O(\witsize\cdot \tabsize)$, and in return reduce prover complexity to a \emph{linear} number of field and group operations!
 \end{enumerate}
 \end{abstract}
...
\begin{adjustbox}{width=1\textwidth}
	...
	\thead{Scheme} & \thead{Preprocessing} & \thead{Proof size} & \thead{Prover Work} & \thead{Verifier Work} & \thead{Homomorphic?} & \thead{Aggregatable?} \\ \hline
		...
		\cq          & $O(\tabruntime)$ \F,\G1     & $3n + 3a$ \G1, $1$ \G2 & \makecell[l]{$11n + 11a$ \G1 exp , \\ $\approx54(n+a)\text{log}(n+a)$ \F mul}        & $7$ \G1, $6$ \F & \cmark & \cmark \\ \hline
