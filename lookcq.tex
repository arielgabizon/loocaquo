...
\paragraph{Notation:}
In the section and the next, we use the following conventions.
...
Given $P\in \F[X]$ and integer $i\in [\bigspace]$, we denote $P_i\defeq P(\vgen^i)$.
...
For a polynomial $A(X)\in \polysofdeg{\tabsize}$, we say it is \witsize-sparse if $A_i\neq 0$ for at most \witsize values $i\in[\tabsize]$.
...
The main result of this section is a method to compute a commitment to a quotient polynomial ``proving'' a product with a preprocessed polynomial $\mod\; Z_\bigspace(X)$, in a number of operations depending only on the degree of the polynomial of the other polynomial in the product. 

\begin{thm}
Fix integer parameters $0\leq n\leq \leq N$ such that $n,N$ are powers of two. Fix $T\in \polysofdeg{\tabsize}$, and a subgroup $\bigspace\subset \F$ of size \tabsize. Let $\srs0=\sett{\enc1{x^i}}{i\in[0,\ldots,\tabsize]}$ for some $x\in \F$. 
There is an algorithm \alg that after a preprocessing step of $O(\tabruntime)$ \G1- and \F-operations starting with \srs0 does the following.
Given input $A(X)\in \polysofdeg{\tabsize}$ that is \witsize-sparse and given in sparse representation, \alg computes in 
$\witsize$ \F- and \G1-operations the element
