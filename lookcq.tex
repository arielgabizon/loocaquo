...
\begin{adjustbox}{width=1\textwidth}
	...
	\thead{Scheme} & \thead{Preprocessing} & \thead{Proof size} & \thead{Prover Work} & \thead{Verifier Work} & \thead{Homomorphic?} & \thead{Aggregatable?} \\ \hline
		...
		\cq (this work)         & $O(\tabruntime)$ \F,\G1     & $8$ \G1, $4$ \F & $8n$ \G1, $O(n\log n)$ \F  & $5P$ & \cmark & \cmark \\ \hline
...
\begin{enumerate}
\item Choose random $x\in \F$ compute and output \sett{\enc1{x^i}}{i\in \set{0,\ldots,\tabsize-1}}. Compute \sett{\enc2{x^i}}{i\in \set{0,\ldots,\tabsize}}.
...
\item\label{step:compB0}  \prv computes $B_0\in \polysofdeg{\witsize-1}$ defined as $B_0(X)\defeq \frac{B(X)-B(0)}{X}$.
...
Note that although the above description contains nine pairings, we can reduce to five pairings via the standard technqiue of randomly batching
...
We also note that the commitment \enc1{\frac{A(x)-A(0)}{x}} required for computing \zerproof, can be computed in \witsize \G1-operations
...
Let $f\in \polysofdeg{\tabsize}$ be the polynomial sent by \adv in the first step of the game
