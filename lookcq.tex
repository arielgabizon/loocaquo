...
\begin{enumerate}
 \item Fix table $T$
 \item Choose srs 
 \item relation is $(\cm,f)$ such that \enc1{f(x)}=\cm and $\restricttoset{f}{\subspace}\subset T$.
 \item \prv sends \cm
 \item \ext outputs $f$
 \item \prv and \ver  \prob that \ver outputs \acc but $\restricttoset{f}{\subspace}\not\subset T$ is \negl
...
Input $(\cm,f)$.
\begin{enumerate}
 \item \prv computes poly $m\in \polysofdeg{\tabsize}$ such that $m_i=$ number of times $t_i$ appears in \restricttoset{f}{\subspace}
 \item \prv sends \enc1{m(x)}.
 \item \ver chooses and sends random $\beta \in \F$.
\item \prv computes $A\in \polysofdeg{\tabsize}$ such that for $i\in [\tabsize]$, $A_i = m_i/(t_i+\beta)$.
\item \prv sends \enc1{A(x)}.
\item\label{step:computeQA} \prv computes $q_A\defeq \enc2{Q_A(x)}$ where $Q_A\in \polysofdeg{\tabsize}$ is such that 
...
\item \prv sends \enc1{B(x)}
\item \prv computes $Q_B(X)$ such that 
\[B(X)(f(x)+\beta)-1 = Q_B(X)\cdot Z_\subspace(X)\]
\item \prv computes and sends the values $a=A(0),b=B(0)$.
\item \ver sends random $\alpha\in \F$.
\item \ver checks that $\tabsize\cdot a = witsize\cdot b$.
...
\item \prv sends $A(\gamma), B(\gamma),Q_B(\gamma),f(\gamma)$.
\item \ver checks $P$ is correct
...
The element $q_A$ in Step \ref{step:computeQA} can be computed in  XX \G2 operations and $O(\witsize\log\witsize)$ \F operations
\end{lemma}
